...
def load_triples(file_name):
    ...
    for line in open(file_name, 'r'):
        ...
        if len(params) == 5:
            ...
            head, r, tail, ts, te = int(params[0]), int(params[1]), int(params[2]), int(params[3]), int(
                params[4])  ###
            entity.add(head);
            entity.add(tail);
            rel.add(r + 1);
...
def get_matrix(triples,entity,rel,time):    #{0, 1, 2, 3, 4] 494
        ...
        if len(triples[0]) < 5:
            ...
            for h,r,t,tau in triples:   #构造实体的邻接矩阵
                ...
                r_t[r] = t
        if len(triples[0]) == 3:
            for h,r,t in triples:   #构造实体的邻接矩阵
                adj_matrix[h,t] = 1; adj_matrix[t,h] = 1;
                adj_features[h,t] = 1; adj_features[t,h] = 1;
                radj.append([h,t,r]); radj.append([t,h,r+rel_size]);    #引入反转关系的四元组列表，反转关系：r+rel_size
                rel_out[h][r] += 1; rel_in[t][r] += 1
        ...
        if len(triples[0]) == 3:
            for h, t, r in sorted(radj, key=lambda x: x[0] * 10e10 + x[
                1] * 10e5):  # x:x[]字母可以随意修改，排序方式按照中括号[]里面的维度进行排序，[0]按照第一维排序，[2]按照第三维排序,这是按第一维排序
            ...
            adj_features = normalize_adj(
                adj_features)  # (19054, 19054),  (0,0) 0.003999999999999999,(0,4)  0.003999999999999999
            rel_features = normalize_adj(
                sp.lil_matrix(rel_features))  # (19054, 988),  (0,2) 2.830936473785528e-05 (0,3) 1.0000000000000002
